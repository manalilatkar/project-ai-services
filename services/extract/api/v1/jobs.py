...
async def extract_sync(request: Request) -> JSONResponse:
    ...
    if schema_row is None:
        raise ExtractException(404, "SCHEMA_NOT_FOUND", f"No schema with id {body.schema_id!r}.")
    ...
    async with concurrency_limiter:
        ...
        try:
            ...
            raise ExtractException(500, "LLM_ERROR", "Unexpected error during LLM call.")
        ...
        try:
            ...
            try:
                ...
                raise ExtractException(500, "LLM_ERROR", "Unexpected error during retry LLM call.")

            retry_choices = retry_resp.get("choices", [])
            if not retry_choices:
                raise ExtractException(
                    500, "LLM_ERROR", "vLLM returned an empty choices list on retry."
                )
