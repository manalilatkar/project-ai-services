...
Covers:
  ...
  - Helper functions: _strip_markdown_fences, _render_few_shot_block,
    _build_messages, _call_vllm, _validate_output
...
from extract.utils.schema import ExtractException
from extract.api.v1.jobs import (
    _build_messages,
...
class TestExtractException:
    def test_attributes_set_correctly(self):
        exc = ExtractException("MY_CODE", "something went wrong", status=409)
    ...
    def test_details_default_empty_dict(self):
        exc = ExtractException("C", "m", status=400)
        assert exc.details == {}

    def test_details_populated(self):
        exc = ExtractException("C", "m", status=422, details={"x": 1})
        assert exc.details == {"x": 1}

    def test_is_exception(self):
        exc = ExtractException("C", "m")
        assert isinstance(exc, Exception)

    def test_handler_serialises_to_json(self, extract_test_client):
        """The app exception handler must return the canonical error envelope."""
        # Hit a stub endpoint that raises ExtractException.
    ...
    def test_handler_includes_details_when_present(self, extract_test_client):
        """
        Patch a stub so it raises ExtractException with details; the handler
        must include the details key.
        """
        with patch(
            "extract.api.v1.jobs.db_repo.get_schema_by_id",
            side_effect=ExtractException("DEMO", "demo", status=400, details={"k": "v"}),
        ):
...
class TestStripMarkdownFences:
    ...
    def test_partial_fence_no_close(self):
        # Only an opening fence without closing — strip leading fence only.
        result = _strip_markdown_fences("```json\n{}")
        assert result == "{}"


# ---------------------------------------------------------------------------
# Helper: _render_few_shot_block
# ---------------------------------------------------------------------------
...
class TestRenderFewShotBlock:
    ...
    def test_single_example_renders_correctly(self):
        ...
        result = _render_few_shot_block(examples)
...
# ---------------------------------------------------------------------------
# Helper: _build_messages
# ---------------------------------------------------------------------------

class TestBuildMessages:
    def test_returns_system_and_user_messages(self):
        msgs = _build_messages(SIMPLE_SCHEMA, "", "some text", None)
        roles = [m["role"] for m in msgs]
        assert roles == ["system", "user"]

    def test_system_contains_extraction_instructions(self):
        msgs = _build_messages(SIMPLE_SCHEMA, "", "text", None)
    ...
    def test_custom_prompt_appended_to_system(self):
        msgs = _build_messages(SIMPLE_SCHEMA, "", "text", "Use ISO 8601 for dates.")
        assert "ISO 8601" in msgs[0]["content"]

    def test_no_custom_prompt_placeholder_empty(self):
        msgs = _build_messages(SIMPLE_SCHEMA, "", "text", None)
        # {custom_prompt} replaced with "" — not left as a literal placeholder
...
# ---------------------------------------------------------------------------
# Helper: _validate_output
# ---------------------------------------------------------------------------

class TestValidateOutput:
    def test_valid_json_passes(self):
        raw = json.dumps(VALID_EXTRACTION)
        result = _validate_output(raw, SIMPLE_SCHEMA)
    ...
    def test_extra_properties_allowed_by_default(self):
        ...
        result = _validate_output(json.dumps(with_extra), SIMPLE_SCHEMA)
...
# (all external boundaries mocked via patch)
# ---------------------------------------------------------------------------

class TestExtractSyncEndpoint:
    """
    Fixtures supplied by conftest.py: extract_test_client.
    All DB / vLLM calls are patched per test.
    """

    # ── common patch helpers ──────────────────────────────────────────────

    def _patch_tokenize(self, tokens: int):
        return patch(
            "extract.api.v1.jobs.asyncio.to_thread",
            new_callable=lambda: lambda *a, **kw: _async_first_call(a, tokens),
        )
    ...
    def test_413_content_length_header_exceeds_limit(self, extract_test_client, monkeypatch):
        monkeypatch.setattr(
            "extract.api.v1.jobs.settings.extract.max_request_body_bytes", 100
        )
        resp = extract_test_client.post(
            "/v1/extract",
    ...
    def test_413_body_exceeds_limit_without_header(self, extract_test_client, monkeypatch):
        monkeypatch.setattr(
            "extract.api.v1.jobs.settings.extract.max_request_body_bytes", 10
        )
        resp = extract_test_client.post(
            "/v1/extract",
            content=b"x" * 100,
    ...
    def test_413_output_budget_exceeded_no_retry(self, extract_test_client, monkeypatch):
        """finish_reason=length must fail fast — _call_vllm must not be called twice."""
        schema_row = _mock_schema_row()
        ...
        async def _mock_to_thread(fn, *args, **kwargs):
            call_count["n"] += 1
            if fn.__name__ == "_tokenize":
                return 200
            if fn.__name__ == "check_extraction_budget":
                return 512
            # _call_vllm
            return length_resp
        ...
        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             _patch_concurrency_free(), \
             patch("extract.api.v1.jobs.asyncio.to_thread", new=AsyncMock(side_effect=_mock_to_thread)), \
             patch("extract.api.v1.jobs.check_extraction_budget", return_value=512), \
             patch("extract.api.v1.jobs._call_vllm", return_value=length_resp):
            resp = extract_test_client.post(
                "/v1/extract",
    ...
    def test_413_output_budget_has_reserved_tokens_in_details(self, extract_test_client, monkeypatch):
        ...
        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             _patch_concurrency_free(), \
             patch("extract.api.v1.jobs.asyncio.to_thread", new=AsyncMock(return_value=200)), \
    ...
    def test_422_validation_fails_after_retry(self, extract_test_client, monkeypatch):
        ...
        good_call_returns_bad = _vllm_response(bad_json)

        vllm_calls = {"n": 0}

        def _fake_call_vllm(*args, **kwargs):
            vllm_calls["n"] += 1
            return good_call_returns_bad  # both calls return invalid output

        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             _patch_concurrency_free(), \
             patch("extract.api.v1.jobs.asyncio.to_thread", new=AsyncMock(return_value=200)), \
             patch("extract.api.v1.jobs.check_extraction_budget", return_value=512), \
             patch("extract.api.v1.jobs._call_vllm", side_effect=_fake_call_vllm):
            resp = extract_test_client.post(
                "/v1/extract",
                json={"text": "some text", "schema_id": "schema-001"},
            )
        ...
        # Exactly two LLM calls made (initial + one retry)
        assert vllm_calls["n"] == 2

    def test_422_retry_output_in_details(self, extract_test_client, monkeypatch):
        schema_row = _mock_schema_row()
        bad_raw = '{"invoice_number": "X"}'  # missing required fields

        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             _patch_concurrency_free(), \
             patch("extract.api.v1.jobs.asyncio.to_thread", new=AsyncMock(return_value=200)), \
             patch("extract.api.v1.jobs.check_extraction_budget", return_value=512), \
             patch("extract.api.v1.jobs._call_vllm", return_value=_vllm_response(bad_raw)):
            resp = extract_test_client.post(
                "/v1/extract",
                json={"text": "x", "schema_id": "schema-001"},
            )
    ...
    def test_validation_attempts_2_when_retry_succeeds(self, extract_test_client, monkeypatch):
        ...
        schema_row = _mock_schema_row()
        bad_json = '{"invoice_number": "X"}'  # missing required fields
        good_json = json.dumps(VALID_EXTRACTION)
        ...
        def _fake_call_vllm(*args, **kwargs):
            ...
            return _vllm_response(good_json)
    ...
    def test_finish_reason_length_on_retry_returns_413(self, extract_test_client, monkeypatch):
        ...
        def _fake_call_vllm(*args, **kwargs):
            calls["n"] += 1
            if calls["n"] == 1:
                return _vllm_response(bad_json)           # invalid → triggers retry
    ...
    def test_429_when_semaphore_locked(self, extract_test_client, monkeypatch):
        ...
        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             patch("extract.api.v1.jobs.concurrency_limiter", locked_semaphore, create=True):
            # Need to also patch the import path used inside the function
            with patch("extract.app.concurrency_limiter", locked_semaphore):
                resp = extract_test_client.post(
                    "/v1/extract",
                    json={"text": "some text", "schema_id": "schema-001"},
                )
    ...
    def test_503_vllm_connection_error(self, extract_test_client, monkeypatch):
        ...
        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             ...
             patch("extract.api.v1.jobs.check_extraction_budget", return_value=512), \
             patch(
                 "extract.api.v1.jobs._call_vllm",
    ...
    def test_500_vllm_http_error(self, extract_test_client, monkeypatch):
        ...
        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             ...
             patch("extract.api.v1.jobs.asyncio.to_thread", new=AsyncMock(return_value=200)), \
             patch("extract.api.v1.jobs.check_extraction_budget", return_value=512), \
             patch("extract.api.v1.jobs._call_vllm", side_effect=http_err):
            resp = extract_test_client.post(
                "/v1/extract",
                json={"text": "hello", "schema_id": "schema-001"},
            )
    ...
    def test_500_empty_choices(self, extract_test_client, monkeypatch):
        schema_row = _mock_schema_row()
        empty_choices_resp = {"choices": [], "usage": {}}

        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             _patch_concurrency_free(), \
             patch("extract.api.v1.jobs.asyncio.to_thread", new=AsyncMock(return_value=200)), \
             patch("extract.api.v1.jobs.check_extraction_budget", return_value=512), \
             patch("extract.api.v1.jobs._call_vllm", return_value=empty_choices_resp):
            resp = extract_test_client.post(
                "/v1/extract",
                json={"text": "x", "schema_id": "schema-001"},
            )
    ...
    def test_guided_json_included_when_enabled(self, extract_test_client, monkeypatch):
        """When GUIDED_DECODING_ENABLED=true, guided_json must appear in vLLM payload."""
        schema_row = _mock_schema_row()
        good_json = json.dumps(VALID_EXTRACTION)
        ...
        monkeypatch.setattr("extract.api.v1.jobs.settings.extract.guided_decoding_enabled", True)
    ...
    def test_guided_json_absent_when_disabled(self, extract_test_client, monkeypatch):
        ...
        import extract.api.v1.jobs as jobs_module
        ...
        real_session_post = None

        def _mock_post(url, json=None, headers=None, stream=False):
            captured_payload.update(json or {})
            mock_resp = Mock()
            mock_resp.raise_for_status = Mock()
            mock_resp.json.return_value = _vllm_response(good_json)
            return mock_resp

        monkeypatch.setattr("extract.api.v1.jobs.settings.extract.guided_decoding_enabled", False)
    ...
    def test_usage_reflects_vllm_token_counts(self, extract_test_client, monkeypatch):
        schema_row = _mock_schema_row()
        good_json = json.dumps(VALID_EXTRACTION)
        vllm_resp = _vllm_response(good_json, prompt_tokens=400, completion_tokens=60)

        with patch("extract.api.v1.jobs.db_repo.get_schema_by_id", return_value=schema_row), \
             _patch_concurrency_free(), \
             patch("extract.api.v1.jobs.asyncio.to_thread", new=AsyncMock(return_value=200)), \
             patch("extract.api.v1.jobs.check_extraction_budget", return_value=512), \
             patch("extract.api.v1.jobs._call_vllm", return_value=vllm_resp):
            resp = extract_test_client.post(
                "/v1/extract",
                json={"text": "x", "schema_id": "schema-001"},
            )
    ...
    def test_usage_accumulates_across_retry(self, extract_test_client, monkeypatch):
        """Token counts from both the initial and retry calls must be summed."""
        schema_row = _mock_schema_row()
        bad_json = '{"invoice_number": "X"}'
        good_json = json.dumps(VALID_EXTRACTION)
        ...
        def _fake_call_vllm(*args, **kwargs):
            calls["n"] += 1
            if calls["n"] == 1:
                return _vllm_response(bad_json, prompt_tokens=300, completion_tokens=50)
            return _vllm_response(good_json, prompt_tokens=400, completion_tokens=80)
        ...
        assert resp.status_code == 200
        usage = resp.json()["usage"]
        assert usage["input_tokens"] == 300 + 400
        assert usage["output_tokens"] == 50 + 80
        assert usage["total_tokens"] == 830

    # ── Stub endpoints still return 501 ───────────────────────────────────

    def test_stubs_return_501(self, extract_test_client):
        for method, path in [
            ("POST", "/v1/extract/jobs"),
            ("GET", "/v1/extract/jobs"),
            ("GET", "/v1/extract/jobs/some-id"),
            ("GET", "/v1/extract/jobs/some-id/result"),
            ("DELETE", "/v1/extract/jobs/some-id"),
            ("DELETE", "/v1/extract/jobs"),
        ]:
            resp = extract_test_client.request(method, path)
            assert resp.status_code == 501, f"{method} {path} expected 501, got {resp.status_code}"
            assert resp.json()["error"]["code"] == "NOT_IMPLEMENTED"


# ---------------------------------------------------------------------------
# Private patch helpers (module-level, reused across test methods)
# ---------------------------------------------------------------------------
...
def _patch_concurrency_free():
    ...
    unlocked = asyncio.BoundedSemaphore(1)
    return patch("extract.app.concurrency_limiter", unlocked)


def _setup_happy_path(monkeypatch, schema_row, input_tokens: int, raw_output: str):
    """
    Wire up all mocks for the standard 200 success flow:
      - db_repo.get_schema_by_id → schema_row
      - concurrency_limiter → unlocked semaphore
      - asyncio.to_thread → input_tokens (for _tokenize)
      - check_extraction_budget → 512
      - _call_vllm → valid vLLM response containing raw_output
    """
    monkeypatch.setattr(
        "extract.api.v1.jobs.db_repo.get_schema_by_id",
    ...
    monkeypatch.setattr(
        "extract.api.v1.jobs.check_extraction_budget",
        Mock(return_value=512),
    )
    monkeypatch.setattr(
        "extract.api.v1.jobs._call_vllm",
        Mock(return_value=_vllm_response(raw_output)),
    )
    monkeypatch.setattr(
        "extract.api.v1.jobs.asyncio.to_thread",
        AsyncMock(return_value=input_tokens),
    )

    import asyncio
    unlocked = asyncio.BoundedSemaphore(1)
    monkeypatch.setattr("extract.app.concurrency_limiter", unlocked)
